...
int decrypt(uint_least8_t* iData, int_least32_t iWidth, int_least32_t iHeight, char mes_res[], const char key[])
{
    ...
    int_least32_t length_mes = read_length_mes(iData, iHeight, iWidth, key) + 8;
    char mes_res_binary[length_mes];

    int_least32_t length_mes2 = length_mes;
    
    for (int_least32_t i=0; i < iHeight; i++)
    {
        if(length_mes2 == 0)
            break;
        for (int_least32_t j=0; j < iWidth; j++)
        {
            pixel = (i * iWidth + j) % 35;
            if(pixel == 0)
                fib = 0;
...
int_least32_t read_length_mes(uint_least8_t* iData, int iHeight, int iWidth, const char key[])
{
    ...
    int_least32_t length_mes = bin_to_int_digit(bit_mes, 8) * 8;
